...
let __secret_recovering () =
  ...
  let checksum, shares = Sh.share ~secret:_SECRET ~threshold ~amount in
  let secret' = Sh.recover ~checksum ~shares in
...
let __secret_recovering_failing () =
  ...
  let procedure () = ignore @@ Sh.recover ~checksum ~shares:shares' in
  Alcotest.check_raises "secrets don't match" reason procedure


let __secret_recovering_format () =
  let checksum = "deadbeef" in
  let shares =
    [ "This is clearly not a valid Base-64 data."
    ; "I mean,\nthis is not fun to test every f#&$ing thing!"
    ]
  in
  let reason = Sh.InvalidSharesFormat in
  let procedure () = ignore @@ Sh.recover ~checksum ~shares in
